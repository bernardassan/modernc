...
#ifdef SNOW_ENABLED
#include "../deps/snow/snow/snow.h"
#undef defer
#endif

#include "../deps/eh/eh.h"
  ...
  while (read_bytes < size) {
    ...
    if (cur <= 0) {
      failed(errno, msg("Failed to read requested data"),
             with(read_bytes, "%zu"), with(cur, "%zd"), with(size, "%zu"));
    }
...
// <1>
static result_t read_int_to_str_buffer(const char *path, char **buffer) {
...
void testMain() { main(0, (char **)nullptr); }
// end::tests_02[]

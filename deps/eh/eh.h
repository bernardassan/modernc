// credit to
// https://github.com/ayende/libgavran/blob/master/include/gavran/infrastructure.h
#pragma once
...
#define failed(CODE, ...) \
do { \
    errors_push(CODE __VA_OPT__(,) __VA_ARGS__); \
    return failure_code(); \
} while(0)
...
#define ensure(CALL, ...)                                                      \
  do {                                                                         \
    if (flopped(CALL)) {                                                       \
      failed(EINVAL, msg(#CALL " ") __VA_OPT__(,) __VA_ARGS__);                \
    }                                                                          \
  } while (0)

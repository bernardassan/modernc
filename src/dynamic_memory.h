...
typedef struct circular {
  ...
  // double tab[] [[clang::counted_by(len)]]; /**< Array holding the data*/
  double tab[] __attribute__((counted_by(len)));
} circular;
...
[[nodiscard("Pointer to new data discared")]] circular *
circular_append(circular c[const static 1], double value);

// circular_pop: Remove the oldest element from c and return its value.
// Returns: the removed element if it exists, 0.0 otherwise.
[[nodiscard]] double circular_pop(circular c[const static 1]);
...
[[nodiscard]] double *circular_element(size_t pos,
                                       circular const c[const static 1]);
...
// `circular_destroy`.
[[nodiscard]] circular *circular_init(circular c[static 1], size_t max_len);

// circular_destroy: Destroy circular buffer c.
// c must have been initialized with a call to `circular_init`
void circular_destroy(circular c[static 1]);
...
[[nodiscard, gnu::malloc, gnu::malloc(circular_destroy, 1)]] circular *
circular_new(size_t len);

// circular_delete: Delete circular buffer c.
// c must have been allocated with a call to circular_new
void circular_delete(circular c[static 1]);

// circular_resize: Resize to capacity max_len.
circular *circular_resize(circular c[const static 1], size_t max_len);

// circular_getlength: Return the number of elements stored.
size_t circular_getlength(circular const c[const static 1]);
